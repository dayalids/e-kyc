...
const Table = ({ title = 'Spaces', items, openForm }) => {
  const [isUserModalOpen, setIsUserModalOpen] = useState(false);
  const [editId, setEditId] = useState();
  // get the data from permission reducer and put it in use callback
  ...
  const COLUMNS = [
    ...
    },
    {
      ...
      accessor: 'flower',
      Cell: (row) => {
        return <span>{row?.cell?.value?.flowerId}</span>;
      },
    },
    {
      Header: 'Petal',
      accessor: 'petal',
      Cell: (row) => {
        return <span>{row?.cell?.value?.title}</span>;
      },
    },
    {
      Header: 'Hexagon Id',
